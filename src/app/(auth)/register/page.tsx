...
import { Heart, Mail, Lock, User, CheckCircle2 } from 'lucide-react'
...
const perks = [
  ...
  'Track income from 10+ source types',
  'Set investment goals with progress tracking',
  'Connect your bank for auto-import',
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterData) => {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: { data: { full_name: data.fullName, currency: data.currency } },
    })
    if (error) {
    ...
    <div className="min-h-screen flex" style={{ background: '#050C18' }}>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md animate-fade-in">
          ...
          <p className="text-slate-400 text-sm mb-8">Already have one?{' '}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <Label className="text-slate-300 text-sm mb-1.5 block">Default Currency</Label>
              <Select onValueChange={v => setValue('currency', v)} defaultValue="USD">
                <SelectTrigger className="h-11 text-white rounded-xl"
                  style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              {errors.currency && <p className="text-rose-400 text-xs mt-1">{errors.currency.message}</p>}
            </div>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] p-12 relative overflow-hidden"
        style={{ background: 'linear-gradient(160deg, #0D0829 0%, #050C18 60%)', borderLeft: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="orb orb-2" style={{ opacity: 0.2 }} />

        <div className="relative z-10 flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl gradient-bg flex items-center justify-center shadow-lg shadow-violet-500/40">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-400 text-base mb-10">Join couples building wealth together.</p>
